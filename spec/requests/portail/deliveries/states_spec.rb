...
RSpec.describe "Portail::Deliveries::States", type: :request do
  ...
  def update_state(state = "done", seen_state: "in_progress")
    patch path, params: {etat: state, etat_vu: seen_state}
  end

  # Le détail relu après la redirection lit le profil de la démarche.
  before do
    stub_organisation_subscriptions
    stub_data_stream_profile
  end
  ...
  describe "PATCH /teledossiers/:teledossier_id/etat" do
    ...
    # Le rechargement qu'on demande à l'agent, c'est la redirection elle-même.
    it "refuses when the delivery moved since the agent saw it" do
      sign_in_member
      serve(times: 2)
      expect(Portail::HubAPI::Deliveries).not_to receive(:change_state)

      update_state(seen_state: "transmitted")
      follow_redirect!

      expect(response).to have_http_status(:success)
      expect(Capybara.string(response.body)).to have_text("a changé depuis l'affichage")
    end
    ...
    it "refuses a move with no state at all" do
      ...
      patch path, params: {etat: "", etat_vu: "in_progress"}
      follow_redirect!
...

    {
      ...
      Portail::HubAPI::AwaitingDocumentsNotAllowed => "n'autorise pas l'attente de compléments",

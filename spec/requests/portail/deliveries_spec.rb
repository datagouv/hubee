...
RSpec.describe "Portail::Deliveries", type: :request do
  ...
  describe "GET /teledossiers/:id" do
    ...
    it "falls back to the code when the data stream has no readable name" do
      sign_in_member
      stub_data_stream_profile(build(:portail_data_stream_profile, name: nil))
      expect(Portail::HubAPI::Deliveries).to receive(:find).and_return(build(:portail_delivery))
    ...
    context "state change" do
      ...
      it "hides the complements request when the data stream forbids it" do
        sign_in_member
        stub_data_stream_profile(build(:portail_data_stream_profile, :without_awaiting_documents))
      ...
      # Sans lui, une page laissée ouverte écraserait le travail d'un collègue en silence.
      it "carries the state the agent is looking at" do
        sign_in_member
        ...
        expect(response).to have_http_status(:success)
        expect(Capybara.string(response.body))
          .to have_field("etat_vu", with: "in_progress", type: :hidden)
      end
    end

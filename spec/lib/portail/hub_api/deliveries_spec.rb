...
RSpec.describe Portail::HubAPI::Deliveries do
  ...
  describe ".change_state" do
    ...
    it "raises a named refusal when the data stream forbids awaiting documents" do
      client = HubApiV1::Testing::FakeClient.new
      client.add_case(build_v2_delivery(state: :in_progress))
      client.add_data_stream_profile(build_v2_data_stream_profile(code: "CERTDC", awaiting_documents: :denied))

      expect { change_state(state: "awaiting_documents", client: client) }
        .to raise_error(Portail::HubAPI::AwaitingDocumentsNotAllowed)
    end

    it "moves the delivery when the data stream allows awaiting documents" do
      client = HubApiV1::Testing::FakeClient.new
      client.add_case(build_v2_delivery(state: :in_progress))
      client.add_data_stream_profile(build_v2_data_stream_profile(code: "CERTDC", awaiting_documents: :allowed))

      expect { change_state(state: "awaiting_documents", client: client) }.not_to raise_error
    end
  ...
  describe "state change error translation" do
    upstream_errors = {
      ...
      "a data stream that forbids awaiting documents" => {
        raised: HubApiV1::V2::AwaitingDocumentsNotAllowedError,
        translated: Portail::HubAPI::AwaitingDocumentsNotAllowed, reported: false
      },

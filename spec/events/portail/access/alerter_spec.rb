...
RSpec.describe Portail::Access::Alerter do
  ...
  it "alerts when the upstream served a page outside the perimeter, naming the page and what was dropped" do
    refusal = Portail::Access::Refusal.new(reason: :upstream_mismatch, path: "/teledossiers",
      membership_id: "m-1", dropped_ids: ["d-1", "d-2"])
    expect(Sentry).to receive(:capture_message).with(
      "Périmètre non respecté par l'amont sur /teledossiers : 2 éléments hors périmètre retirés de la page",
  ...
  it "alerts on a policy refusal, naming the page and who asked" do
    refusal = Portail::Access::Refusal.new(reason: :out_of_perimeter, path: "/teledossiers/x",
      agent_id: "a-1", membership_id: "m-1")
    expect(Sentry).to receive(:capture_message).with(
      "Accès refusé hors périmètre sur /teledossiers/x",
